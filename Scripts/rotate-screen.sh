...
TouchscreenDevice='"SYNAPTICS Synaptics Touch Digitizer V04"'
PenDevice='"Wacom ISDv4 EC Pen stylus"'
...
if  [ "$1" == "-n" ]
    then 
    xrandr -o normal
    echo xinput set-prop "$TouchscreenDevice" \'Coordinate Transformation Matrix\' $normal
    xinput set-prop "$TouchscreenDevice" \'Coordinate Transformation Matrix\' $normal
    echo xinput enable "$TouchpadDevice"
    xinput enable "$TouchpadDevice"

else
    echo "90° to the right"
    echo xrandr -o right
    echo xinput set-prop "$TouchscreenDevice" 'Coordinate Transformation Matrix' $right \&\& xinput set-prop "$TouchpadDevice" 'Coordinate Transformation Matrix' $right \&\& xinput set-prop "$PenDevice" 'Coordinate Transformation Matrix' $right
